...
def counter_home(request):
    ...
    # Group orders by table number and aggregate item quantities and total price
    orders_by_table = {}
    table_totals = {}
    ...
    for order in confirmed_orders:
        ...
        item_price = order.total_price  # Assuming 'total_price' is the price per unit

        # Calculate the total item price (quantity * unit price)
        total_item_price = item_quantity * item_price

        # If the item already exists for the table, add the quantity and update the price; otherwise, set it
        if item_name in orders_by_table[table_number]:
            ...
            orders_by_table[table_number][item_name] = {
                ...
                'price': item_price,  # price per unit
                'total_price': total_item_price  # total price for the item (quantity * price)
            }

        # Update the table total (sum of all item total prices)
